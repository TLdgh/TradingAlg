...
for (i in 1:length(FutToBePrepared)){
  ...
  CombFileLoc_Fut<-paste0("/Users/tengli/CandleStickComb/", FutToBePrepared[[i]][1,"Symb"], "/", nam[i], "Comb.csv") #read the combined data
  ...
  if(FutToBePrepared[[i]][1,"barSize"]!="Continuous"){
    source("Script/IntradayContract.R")   #Get the original futures intraday data
